...
func (sys *System) ComputeDistortion(eye int, u, v float32, dest *DistortionCoordinates) bool {
	...
	if convertCBool2Int(C.system_ComputeDistortion(sys.ptr, C.EVREye(eye), C.float(u), C.float(v), &cDest)) == 0 {
		return false
	}
...
func (sys *System) IsTrackedDeviceConnected(deviceIndex uint32) bool {
	if convertCBool2Int(C.system_IsTrackedDeviceConnected(sys.ptr, C.TrackedDeviceIndex_t(deviceIndex))) != 0 {
		return true
	}
	return false
}
...
func (sys *System) IsInputFocusCapturedByAnotherProcess() bool {
	if convertCBool2Int(C.system_IsInputFocusCapturedByAnotherProcess(sys.ptr)) != 0 {
		return true
	}
	return false
}
