...
func (sys *System) ComputeDistortion(eye int, u, v float32, dest *DistortionCoordinates) bool {
	...
	if C.system_ComputeDistortion(sys.ptr, C.EVREye(eye), C.float(u), C.float(v), &cDest) == false {
		return false
	}
...
func (sys *System) IsTrackedDeviceConnected(deviceIndex uint32) bool {
	if C.system_IsTrackedDeviceConnected(sys.ptr, C.TrackedDeviceIndex_t(deviceIndex)) != false {
		return true
	}
	return false
}
...
func (sys *System) IsInputFocusCapturedByAnotherProcess() bool {
	if C.system_IsInputFocusCapturedByAnotherProcess(sys.ptr) != false {
		return true
	}
	return false
}
