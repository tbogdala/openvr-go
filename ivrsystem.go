...
func (sys *System) ComputeDistortion(eye int, u, v float32, dest *DistortionCoordinates) bool {
	...
	if C.system_ComputeDistortion(sys.ptr, C.EVREye(eye), C.float(u), C.float(v), &cDest) == C.bool(0) {
		return false
	}
...
func (sys *System) IsTrackedDeviceConnected(deviceIndex uint32) bool {
	if C.system_IsTrackedDeviceConnected(sys.ptr, C.TrackedDeviceIndex_t(deviceIndex)) != C.bool(0) {
		return true
	}
	return false
}
...
func (sys *System) IsInputFocusCapturedByAnotherProcess() bool {
	if C.system_IsInputFocusCapturedByAnotherProcess(sys.ptr) != C.bool(0) {
		return true
	}
	return false
}
